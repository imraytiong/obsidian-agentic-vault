	...
	registerTriggers() {
		...
		this.app.workspace.on('file-open', async (file: TFile | null) => {
			...
			if (content.includes('#sherpa-trigger')) {
				void this.logger.log('ASYNC_TRIGGER_DETECTED', { file: file.path });
		...
		this.app.workspace.onLayoutReady(() => {
			setTimeout(() => {
				void (async () => {
					void this.logger.log('BACKGROUND_JOB_STARTED', { job: 'map_vault' });
					const result = await this.sandbox.executeTool('map_vault', { targetPath: '.' });
					if (result.success) {
						void this.logger.log('BACKGROUND_JOB_COMPLETED', { job: 'map_vault', bytes: result.output.length });
					} else {
						void this.logger.log('BACKGROUND_JOB_FAILED', { job: 'map_vault', error: result.output });
					}
				})();
