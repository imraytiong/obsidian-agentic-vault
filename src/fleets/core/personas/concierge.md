...
- **For Install Career Fleet**: First, use the `install_fleet` tool with `fleet_name: "para_career"`. The tool will return a list of required zones. Second, read that list and allocate the EXACT zones required using ONE CALL to the `allocate_zones` tool. For the `agentic_vault` zone, note that this will seamlessly rename the existing configuration folder. You do not need to do anything special besides using `allocate_zones` to point it to its new path. Finally, let them know you've installed the fleet and configured all the zones.
